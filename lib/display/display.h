...
public:
  ...
  INLINE constexpr int16_t max_x() { return R & EX_X_Y ? Driver::max_y() : Driver::max_x(); }
  INLINE constexpr int16_t max_y() { return R & EX_X_Y ? Driver::max_x() : Driver::max_y(); }

  INLINE void color(typename Driver::RGB c) { _color = c; }
  INLINE void color2(typename Driver::RGB c) { _color2 = c; }
...
public:
  ...
  {
    point_x = _viewport->min_x + x;
    point_y = _viewport->min_y + y;
  }
  INLINE void set_interline(uint8_t interline) { _interline = _font.height + interline; }
  INLINE void set_interval(uint8_t interval) { _interval = interval; }
  INLINE uint8_t get_height() { return _interline; }
  INLINE uint8_t get_weight() { return _font.weight + _interval; }

  NOINLINE void putc(uint8_t ch)
  {
    switch ((uint8_t)ch) {
      ...
      case '\e':
        break;
      case '\1': {
          typename Driver::RGB tmp = _color;
          _color = _color2;
          _color2 = tmp;
        } break;
      ...
      default: {
          ...
          if (_font.offset) {
            source = (uint8_t *)_font.data + pgm_read_word(&_font.offset[ch]);
          }
          else {
          ...
          if (point_y + _font.height > _viewport->max_y + 1) {
            point_x = _viewport->min_x;
            point_y = _viewport->min_y;
          }
