...
#include <stdint.h>
  ...
  static inline _int128_t int128_from_int64(int64_t x) { return (_int128_t) { (x < 0 ? -1 : 0), (uint64_t)x }; }
  ...
  {
    int32_t hi[2] = { (int32_t)(x >> 32), (int32_t)(y >> 32) };
    uint32_t lo[2] = { (uint32_t)(x & 0xFFFFFFFF), (uint32_t)(y & 0xFFFFFFFF) };
  ...
  static inline _int128_t int128_mul_i128_i64(_int128_t x, int64_t y)
  {
    int neg = ((x.hi ^ y) < 0);
    if (x.hi < 0)
      x = int128_neg(x);
    ...
    uint64_t _x[4] = { (uint64_t)(x.hi >> 32), (uint64_t)(x.hi & 0xFFFFFFFF), (uint64_t)(x.lo >> 32), (uint64_t)(x.lo & 0xFFFFFFFF) };
    uint64_t _y[2] = { (uint64_t)(y >> 32), (uint64_t)(y & 0xFFFFFFFF) };
